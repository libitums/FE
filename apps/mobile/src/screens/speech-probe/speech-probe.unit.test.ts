...
//
// **이 파일이 지지 않는 것: 판정입니다.** 「이 정도면 맞게 읽은 것이다」를
// 단언하는 케이스가 여기 없습니다. 규칙을 아직 정하지 않았기 때문이고(모듈 상단
// 주석), 테스트가 먼저 규칙을 박으면 구현이 그것을 따라가서 탐침이 관찰하려던
// 것이 사라집니다. 여기서 보는 것은 「후보를 적용하면 문자열이 어떻게 되는가」와
// 「그때 두 줄이 붙는가」까지입니다.
//
// 케이스의 문자열은 한국어 발화에서 실제로 갈리는 자리를 고른 것입니다 —
// 문장부호를 주는지, 띄어쓰기를 어떻게 끊는지, 아무것도 못 알아들었는지.
...
function comparisonById(comparisons: readonly SpeechComparison[], id: string): SpeechComparison {
  ...
  // 못 찾으면 여기서 멈춥니다 — `undefined`에 건 단언은 그 다음 줄에서 알아보기
  // 어려운 메시지로 터집니다.
  if (found === undefined) {
...
// N1 — 목록을 배열째로 대조합니다. 개수만 세면 어느 조합이 빠졌는지 러너가 말하지
// 못합니다.
...
// N2 — id는 화면의 행 키이자 `data-testid`라 겹치면 안 됩니다.
test("후보의 id는 서로 다르다", () => {
...
// N3 — 라벨은 두 축의 낱말을 가운뎃점 양옆 공백으로 잇습니다.
...
// NS2 — 양끝만 건드립니다. 안쪽의 연속 공백은 이 칸의 것이 아닙니다.
...
// NS5 — 문장부호를 낱자 목록으로 세지 않고 유니코드 구두점 범주에 맡긴 결과를
// 봅니다.
...
// NS5b — ⭐ 이 단위가 지키려는 것이 여기 있습니다. 인식기가 한글 자판 밖의 부호를
// 실어 와도 같은 범주로 잡혀야 합니다 — 전각 쉼표·마침표·물음표, CJK 마침표·쉼표,
// 낫표, 반각 가운뎃점. ASCII만 보는 구현이었다면 이 줄이 통째로 남았을 것입니다.
...
// NS5c — 따옴표와 줄표류입니다. 받아쓰기가 곧잘 실어 오는 모양이고, 곧은 따옴표가
// 아니라 굽은 따옴표라 낱자 목록을 적었다면 가장 먼저 새어 나갔을 자리입니다.
...
// NS6 — ⭐ 순서가 결과를 가릅니다. 공백을 먼저 다뤘다면 쉼표가 빠진 자리에 공백이
// 둘 남아 `collapse`가 그것을 보지 못했을 것입니다.
test("문장부호를 먼저 지워서 그 자리에 남은 공백을 collapse가 본다", () => {
  expect(normalizeSpeechText("안녕 , 세상", collapseRemove)).toBe("안녕 세상");
});

// NS7 — ⭐ 빈 문자열은 오류가 아니라 유효한 입력입니다. 인식기가 돌았는데 관측이
// 0건인 것이 실기에서 볼 만한 관찰이라, 어느 후보에서도 그대로 빈 문자열로
// 남습니다.
...
// NS8 — 공백만 실려 온 것과 아무것도 안 실려 온 것이 `trim` 아래에서 같아집니다.
// 둘을 가르는 것은 `speechTextShape`의 몫입니다.
...
// SH5 — `collapse`가 할 일이 있는지를 이 값이 말합니다.
...
// C1 — 행은 후보 목록을 그대로 따릅니다. 화면이 고를 자리가 없습니다.
...
// C2 — 제시문과 인식 결과가 같은 후보를 지납니다. 서로 다른 경로로 정규화되면
// 나란히 놓아도 비교가 안 됩니다.
...
// C3 — ⭐ 인식기가 문장부호를 안 준 경우입니다. 문장부호를 그대로 둔 후보에서는
// 두 줄이 갈리고, 지운 후보에서는 붙습니다. **어느 쪽이 정답인지는 여기서 정하지
// 않습니다.**
...
// C4 — ⭐ 띄어쓰기만 갈린 경우입니다. 공백을 전부 지운 후보에서만 붙습니다.
...
// C5 — ⭐ 인식기가 아무것도 못 알아들은 경우입니다. 빈 문자열이 유효한 입력이라
// 행은 그대로 서고, 제시문이 비어 있지 않으므로 어느 후보에서도 붙지 않습니다.
...
// C6 — ⭐ 둘 다 빈 문자열이면 참이 됩니다. **이것이 「맞았다」가 아닙니다** — 이
// 값은 문자 그대로 같은가일 뿐이고, 그 자리를 무엇으로 가를지는 실기를 보고
// 정합니다.
...
// U1 — ⭐ 이 단위의 핵심 단언입니다. 구현은 `/\p{P}/u`를 쓸 수 없어(모듈 주석: 호스트
// 번들의 바이트코드 인코더가 `\p{...}`를 받지 않습니다) 같은 범주를 코드포인트
// 범위로 펴 두었는데, 그 범위가 정말 유니코드 구두점 범주인지는 눈으로 확인할 수
// 없습니다. 그래서 **범위를 손으로 센 기대값과 맞추지 않고, 유니코드 표 자체와
// 맞춥니다** — vitest는 Node에서 돌아 `\p{P}`를 쓸 수 있습니다. 여기가 어긋나면
// 누군가 범위를 줄이거나 늘린 것입니다.
//
// 개수를 적지 않는 것은 일부러입니다. 유니코드 판이 올라 부호가 늘면 개수를 적은
// 단언은 「늘었다」만 말하고 어느 글자인지는 말하지 않습니다.
...
// U2 — ⭐ **알려진 좁힘을 문면에 남깁니다.** 보조 평면(U+10000 이상)의 구두점은
// 범위에 들어 있지 않습니다 — 쐐기문자·이집트 상형문자 같은 역사 문자의 부호라
// 한국어 인식기에서 올 자리가 없다고 보고 뺐습니다. 이 테스트가 깨진다면 누군가
// 그 평면을 덮은 것이고, 그때는 모듈 주석의 「덮지 못한 것」도 같이 고쳐야
// 합니다.
