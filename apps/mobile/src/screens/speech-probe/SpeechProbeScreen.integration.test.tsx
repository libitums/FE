...
// 맞물리는 것은 화면 · `lib/speech-recognition.ts`의 접점 · 전역 `NativeModules`다.
// **모듈이 없을 때 던지지 않는가**는 고립 렌더로 볼 수 없다 — 접점과 화면이 같은 트리에
// 서야 비로소 관측된다. 형태의 정본:
// ../handwriting-probe/HandwritingProbeScreen.integration.test.tsx
//
// ⚠ **integration 파일이 `src/app/` 밖에 있는 둘째 사례다.** 손글씨 탐침과 같은 이유다 —
// 이 탐침은 **App에서 도달 불가라는 것이 정의**라 App을 무대로 쓰면 그 정의를 깨야 한다.
//
// ⚠ **`vi.stubGlobal("NativeModules", …)`은 전역을 통째로 갈아치운다.** 이 화면의 트리는
// 말하기 모듈 하나만 만지므로 여기서는 안전하다.
//
// 목킹하지 않는다 — 대역을 두는 자리는 **호스트 경계 하나**다. `lib/speech-recognition.ts`를
...
// 네이티브가 실제로 하는 일(오디오 세션·SFSpeechRecognizer)은 이 계층의 관심이 아니다.
// 여기서 보는 것은 **무엇이 건너갔고 무엇이 돌아왔을 때 화면이 어떻게 되는가**뿐이라,
// 대역은 인자를 적어 두고 콜백을 테스트 손에 쥐여 준다 — 콜백을 언제 부를지가 곧 케이스다.
...
// 상태 페이로드의 키 이름은 Swift 쪽 `statusPayload()`를 글자 그대로 흉내 낸다.
...
// 결과 페이로드도 마찬가지로 Swift 쪽 `resultPayload(status:session:)`를 흉내 낸다.
...
test("[IS1] 모듈이 있으면 상태 읽기가 건너가고 권한 둘이 각각 화면에 선다", () => {
  ...
  // ⭐ 거부 조합이 화면에서 갈린다 — 한쪽만 거부된 상태가 두 줄로 따로 보인다.
...
test("[IS2] restricted가 denied로 접히지 않고 화면까지 그대로 온다", () => {
  ...
  // 두 줄이 같은 글자를 내면 그 갈림이 실기 기록에서 사라진다.
...
test("[IS3] 거부 조합에서도 권한 요청 수단이 살아 있고 요청이 건너간다", () => {
  ...
  // ⭐ 화면이 조합을 보고 수단을 감추지 않는다 — 감추는 것이 곧 거부 조합의 처방이고,
  // 그것은 이 단위가 정하지 않는다. 미요청인 것에만 묻는 것은 네이티브가 진다(D5).
  ...
  // 요청이 돌려주는 모양이 조회와 같아서 같은 두 줄이 갱신된다.
  expect(screen.getByTestId("speech-probe-screen-microphone")).toHaveAttribute(
    "data-permission",
    "denied",
  );
...
test("[IS5] start가 온디바이스 요구를 그대로 실어 보내고 토글이 그 값을 바꾼다", () => {
  ...
  // 키가 **정확히 하나**다. 네이티브가 읽지 않는 값을 넘기면 계약에 두 번째 정본이 생긴다.
...
test("[IS7] 빈 문자열을 받은 것과 아직 아무것도 안 온 것이 화면에서 갈린다", () => {
  ...
  // ⭐ 받은 상태 — **본문은 여전히 비어 있다.** 눈으로는 위와 같고, 갈리는 자리는
  // `data-received` 하나다. 빈 문자열을 오류로 접으면 「듣고도 못 읽었다」가 사라진다.
  expect(recognized().textContent).toBe("");
...
test("[IS8] 요청한 온디바이스와 보장된 온디바이스가 갈리는 자리가 보인다", () => {
  ...
  // ⭐ 요청은 했는데 기기가 못 한 자리다. 넷이 함께 서므로 「요청을 안 했다」와
  // 「요청했는데 기기가 못 한다」가 갈린다 — 조용한 서버 인식이 이 탐침의 물음이다.
  ...
  // 요청 자체를 안 한 경우에는 같은 `not-guaranteed`라도 갈림이 없다.
  tap("speech-probe-screen-start");
  act(() =>
...
test("[IS9] 세션 집계가 조회 시점 값과 따로 선다", () => {
  ...
  // 조회 줄은 아직 아무것도 안 읽었으므로 비어 있다 — 두 줄이 다른 시점을 답한다.
...
test("[IS10] 인식 결과가 후보별 대조 표에 그대로 들어가고 화면이 판정하지 않는다", () => {
  ...
  tap("speech-probe-screen-start");
  // 제시문을 문자 그대로 되돌려 준다 — 어느 후보에서도 둘이 붙는 것이 관찰이다.
  ...
  // ⭐ 그래도 화면 어디에도 「맞았다」가 없다. `identical`은 관찰값이지 판정이 아니다.
...
test("[IS12] 전역은 있는데 모듈 키가 없으면 던지지 않고 unavailable로 선다", () => {
  ...
  // ADR-0026 D3의 1단계에서 끝났다 — 권한 줄이 빈 것이 「물을 자리조차 없었다」이고,
  // 그 사실이 보이지 않으면 사람이 2단계의 침묵과 구분하지 못한다.
...
test("[IS13] 전역이 아예 없어도 렌더와 조작이 ReferenceError 없이 지나간다", () => {
  // 앵커 — 전역이 실제로 없다는 것을 먼저 박는다. 맨 식별자 접근이 `ReferenceError`를
  // 던지는 것이 이 환경의 사실이고, `typeof` 가드가 막는 것이 바로 그것이다.
...
test("[IS14] 결과 페이로드가 어그러져도 던지지 않고 malformed로 선다", () => {
  ...
  // 결과는 왔다 — 빈 문자열을 받은 것이지 안 온 것이 아니다.
...
test("[IS15] 결과를 기다리는 동안 연타는 요청을 늘리지 않고, 결과가 오면 다시 열린다", () => {
  ...
  // 연타 방어는 화면의 지역 상태가 진다 — 접점에 두면 진실이 둘이 된다.
  tap("speech-probe-screen-start");
  tap("speech-probe-screen-start");
  expect(host.startCalls).toHaveLength(1);
...
test("[IS16] 멈추기는 네이티브 stop을 부르고 기다림을 스스로 풀지 않는다", () => {
  ...
  // 멈춘 뒤에도 인식기가 마지막 버퍼를 읽어 결과를 확정한다 — 그 확정이 오기 전에
  // 잠금을 열면 요청이 겹친다.
  expect(screen.getByTestId("speech-probe-screen-start")).toHaveAttribute("data-awaiting", "true");
