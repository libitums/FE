...
// 계약: .agent-harness/work/lib-263/spec.md §5 · §6.2 (가) 갈래의 SC1~SC8.
//
// `ui` 계층: 렌더 결과와 상호작용만 본다 (ADR-0006 D4). 계산된 스타일·레이아웃은
// jsdom이 계산하지 않으므로 `toBeVisible`·`toHaveStyle`·`toHaveClass`를 쓰지 않는다
// (vitest.setup.ts).
//
// ⚠ **전역 목록은 특정 이름을 단언하지 않는다.** 테스트 환경의 전역 셋과 실기의
// 전역 셋이 다르고, 같은 실기 안에서도 메인 스레드와 백그라운드 스레드가 다르다.
// 여기서 볼 수 있는 것은 **자기 정합성**뿐이다 — 「몇 개라고 적었는가」와 「몇 줄을
// 그렸는가」가 같고 정렬돼 있다는 것. 실제 이름 확인은 `docs/e2e/`의 수동 항목이 진다.
//
// ⚠ 터치 페이로드의 한계는 `DrawingSurface.ui.test.tsx` 머리와 같다 — 환경이 필드
// 이름을 검증하지 않으므로 이 파일의 초록이 실기의 필드 이름을 보증하지 않는다.

// SC1
test("[SC1] 스크롤 영역이 정확히 하나이고 이름·속성·자식이 ADR-0022 3분할 그대로다", () => {
  const { container } = render(<HandwritingProbeScreen />);

  // 중첩 스크롤 0건 — 화면당 스크롤 영역은 정확히 하나다(ADR-0022).
  ...
  // 클래스와 `data-testid`가 같은 문자열이다(계약 §5.4).
  ...
  // 스크롤 설정 축은 저 둘뿐이다 — `bounces`·`enable-scroll`을 쓰지 않는다.
  expect(
    [...scroll.attributes]
...
test("[SC3] 액션 행에 표면·지우기·읽기·결과 줄·계수 줄이 그 순서로 있다", () => {
  ...
  // `<wrapper>`로 감싼다(표면이 그 경우다). 그래서 세는 것은 이름이 붙은 것들의
  // **문서 순서**다 — `querySelectorAll`이 문서 순서로 준다.
  expect(
  ...
  // ⭐ 그리기 표면은 스크롤 영역 **밖**의 고정 영역에 있다(계약 §5.4의 결정).
  const scroll = screen.getByTestId("handwriting-probe-screen-scroll");
...
test("[SC5] 표면에서 획 하나를 그리면 화면의 획 수가 는다", () => {
  ...
  // 화면이 든 끝난 획이 표면으로 다시 내려간다 — 진실이 하나다(계약 §2.4).
...
test("[SC6] 지우기가 획 수·취소 수·결과를 초기값으로 되돌린다", () => {
  ...
  // `toHaveTextContent("")`은 jest-dom이 거절한다(빈 문자열은 늘 맞으므로) —
  // 본문이 실제로 비었는지는 `textContent`를 직접 본다.
...
test("[SC7] touchcancel이 취소 수를 늘린다", () => {
  ...
  const counts = screen.getByTestId("handwriting-probe-screen-counts");
  expect(counts).toHaveAttribute("data-cancels", "1");
  // 취소된 획은 확정되지 않는다 — 그 신호가 획 속에 묻히면 Q1이 답을 못 낸다.
  expect(counts).toHaveAttribute("data-strokes", "0");
});
...
test("[SC8] 표면에 accessibility-*가 하나도 없다", () => {
  ...
  // 존재 앵커로 표면을 먼저 잡는다 — 부재 단언만 남기면 오타로도 초록이 된다.
  const surface = screen.getByTestId("drawing-surface");
