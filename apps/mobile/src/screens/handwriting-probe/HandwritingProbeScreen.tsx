...
// LIB-263 (ui): 계약 .agent-harness/work/lib-263/spec.md §5.
//
// **개발용 탐침이고 사용자 도달 경로가 0건이다.** 어느 코드도 이 화면을 push하지
// 않는다 — 여기 닿으려면 `navigation.ts`의 `handwritingProbeNav`를 부팅 상태로 손수
// 바꿔 끼워야 한다(§5.6). 바텀 네비게이션·여정 맵 어디에도 진입점이 없다.
//
// ADR-0022 3분할: [고정] 머리 · [흐름] `<scroll-view>` · [고정] 액션 행.
// ⭐ 그리기 표면은 **액션 행**에 있다 — 스크롤 영역 안에 두면 표면과 스크롤이 같은
// 제스처를 다투게 되고, 그 다툼은 이 탐침이 답하기로 한 축이 아니다(§5.4). 표면을
// 고정 영역에 둬서 우회했다는 사실이 이 자리의 결정이다.
//
// 「액션 행」의 이름과 내용이 갈린다: ADR-0022 D1은 액션 행을 「나아가는 수단」으로
// 적는데 탐침에는 나아갈 곳이 없고 **조작**만 있다. 새 고정 영역을 만들지 않는 쪽을
// 골랐다 — 그래야 3분할이 글자 그대로 서고 「스크롤 영역은 정확히 하나」가 지켜진다.

/**
 * 결과 줄 한 자리가 싣는 값.
 *
 * 접점 모듈은 「요청이 갔는가」(`HandwritingReadRequestOutcome`)와 「결과가
 * 무엇인가」(`HandwritingReadOutcome`)를 **다른 값**으로 가른다(§4.4) — 같은 사실을
 * 두 자리가 지지 않게 하려는 결정이다. 그 둘을 사람이 읽을 한 줄로 합치는 것은
 * **표시의 선택**이므로, 접점의 union에 멤버를 더하지 않고 여기 지역 타입으로 만든다.
 */
...
export function HandwritingProbeScreen(): ReactNode {
  ...
  // ⚠ 이 목록은 **이 컴포넌트가 도는 스레드의 전역**이다 — ReactLynx에서 컴포넌트
  // 본문은 백그라운드 스레드에서 돌므로 메인 스레드(worklet)가 보는 전역 셋과 다르다.
  // 실기에서 눈으로 읽을 때 그 차이를 잊지 않도록 여기 적는다.
  //
  // 계산을 순수 모듈에 넣지 않는 근거: 입력이 환경이라 결정적 기대값을 적을 수 없고,
  // unit에 넣으면 공허한 케이스가 된다. ui는 **자기 정합성**(적은 수 = 그린 줄 수,
  // 오름차순)까지만 본다.
  ...
  const read = () => {
    // 연타 방어는 화면이 진다(§4.5) — 접점 모듈에 상태를 두면 진실이 둘이 된다.
    if (reading) {
      return;
    }
    ...
    // 모듈이 없으면 `onResult`가 아예 오지 않는다 — 요청이 안 간 것을 결과가 아니라
    // 요청의 반환값이 답한다. 그 사실을 사람이 읽을 수 있게 줄에 세운다.
  ...
  return (
    <view className="handwriting-probe-screen">
      ...
      {/* [흐름] 스크롤 — 화면당 정확히 하나, 중첩 없음. 클래스와 `data-testid`가 같은
          문자열이다. 속성은 `scroll-orientation`·`scroll-bar-enable` 둘뿐이고
          `bounces`·`enable-scroll`을 쓰지 않는다. `accessibility-*`를 붙이지 않는다 —
          조작 단위가 아니라 상자다(ADR-0022 D5). */}
      <scroll-view
      ...
      >
        {/* 직계 자식은 이것 하나뿐이다(ADR-0022 D4). 전역 목록이 한 벌로 두 값을
            한다: 표면이 스크롤과 다투지 않는다는 관측에는 **다툴 스크롤이 실제로
            있어야** 하고, 동시에 ADR-0017 D1 조건 2의 「JS 런타임 전역 셋」을
            실기에서 눈으로 확인하는 수단이 된다. */}
        <view
      ...
      {/* [고정] 액션 행 — `<scroll-view>` 밖의 화면 직계 자식이다(ADR-0022 D1). */}
      <view
      ...
      >
        ...
        {/* 지우기는 표면의 책임이 아니다 — 끝난 획의 주인이 화면이라서다(§2.8).
            실기에서 Q1~Q3을 여러 번 시도하려면 앱을 다시 켜지 않고 비울 수단이 있어야 한다. */}
        <view
        ...
        {/* 결과 줄 — `data-status`가 판정의 자리다. `read`+빈 문자열 / `read`+틀린 글자 /
            `language-unsupported` / `failed`가 **각각 다른 답**이라 접지 않는다. */}
        <text
        ...
        {/* 계수 줄 — 사람이 실기에서 읽는 관측값이다. 획이 하나도 안 보일 때
            `data-strokes`가 느는지가 「좌표는 왔는데 렌더가 문제」와 「바인딩이 안
            닿는다」를 가르고, `data-cancels`가 시스템이 제스처를 가져가는지를 보인다. */}
        <text
