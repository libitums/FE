...
// 않습니다 (vitest.setup.ts). 그어진 **픽셀**도 여기서 볼 수 없습니다 — 판정
// 대상은 「무엇이 그려졌는가」가 아니라 「무엇이 컴포넌트 상태와 렌더 구조에
// 반영됐는가」입니다.
//
// ⚠ **이 파일이 통과해도 실기의 필드 이름이 맞다는 보장이 없습니다.** 테스트
// 환경은 발화 페이로드의 필드 이름을 검증하지도 해석하지도 않고, 실어 준 키를
// 그대로 핸들러에 얹습니다. 그래서 여기 실은 `touches[0].x`는 **구현이 읽기로
// 계약한 이름**을 그대로 따라 적은 것일 뿐이고, 실제 Lynx 런타임이 같은 이름을
// 채우는지는 `docs/e2e/`의 수동 확인이 집니다.
//
// ⚠ 노출용 `data-` 속성은 하이픈 없는 한 낱말입니다. 하이픈을 더 넣으면 렌더가
// `SyntaxError`로 터지고 같은 파일 뒤 테스트까지 연쇄로 깨집니다.

// 계약이 문자열로 못박은 빈 문서입니다. 여기서 `strokesSvgDocument`를 불러
// 기대값을 만들면 「구현이 자기가 부른 함수와 같다」는 공허한 단언이 됩니다 —
// 계약의 글자를 그대로 적습니다.
const emptyDocument =
...
test("[DS2] 초기 content가 계약의 빈 문서 문자열과 같다", () => {
  renderSurface({});
...
test("[DS3] touchstart 하나가 점 하나짜리 진행 획을 세운다", () => {
  ...
  // 점 하나짜리 획은 자기 자신으로 가는 `L`을 달아 눈에 보이는 점획이 됩니다.
...
test("[DS12] 진행 중인 획 위에 새 touchstart가 오면 진행분이 취소로 보고되고 새 획이 선다", () => {
  ...
  fireEvent.touchstart(surface, { touches: [{ x: 1, y: 1 }] });
  fireEvent.touchmove(surface, { touches: [{ x: 2, y: 2 }] });
  // 획을 긋는 도중에 둘째 손가락이 닿는 자리입니다 — 손가락을 떼지 않았으므로
  // `touchend`가 아니라 `touchstart`가 다시 옵니다.
  fireEvent.touchstart(surface, { touches: [{ x: 7, y: 7 }] });

  // 끼어듦은 **중단**이지 완성이 아닙니다. 완성으로 올리면 사용자가 긋지 않은 획이
  // 글자에 섞여 인식 결과가 오염되고, 아무것도 올리지 않으면 모은 점이 어느
  // 계수도 늘리지 않고 조용히 사라져 계수 줄이 관측 도구 구실을 못 합니다.
  expect(onStrokeCancel).toHaveBeenCalledTimes(1);
  expect(onStrokeComplete).not.toHaveBeenCalled();

  // 그리고 새 획은 실제로 시작됩니다 — 버리기만 하고 마는 것이 아닙니다.
  expect(surface).toHaveAttribute("data-points", "1");
