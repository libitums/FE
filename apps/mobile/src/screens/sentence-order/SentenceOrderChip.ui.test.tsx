...
//
// 놓임/안 놓임은 두 값입니다 — `placedOrdinal === null`(창고) / `placedOrdinal`이
// 수(문장 줄)입니다. 판정은 이 컴포넌트에 붙지 않습니다 — 조각은 정오를 모릅니다.

// ------------------------------------------------------------- 채널 1: 상태 `data-*`

// 창고(placedOrdinal === null)에서 data-placed가 "none"입니다. 조건부로 속성을
// 빼지 않습니다 — "속성을 붙이는 것을 잊었다"와 "놓이지 않았다"가 구별되어야
// 합니다.
...
// 배치되면 data-placed가 1-based 자리 번호 문자열입니다.
...
// 창고에서는 접미사가 없습니다 — 라벨이 text와 정확히 같습니다(ADR-0016 D3: 비선택은
// 이름만).
...
// 배치되면 접미사 ", N번째"가 붙습니다. 구분자는 쉼표 + 공백입니다(ADR-0016 D3).
...
// 두 상태 다 조작 단위의 이름·역할이 붙습니다. `disabled`로 바뀌지 않습니다
// (ADR-0016 D10 — 채점 뒤 조각은 다음 문항에서 다시 눌립니다).
...
// ADR-0016 D3: 상태는 라벨 접미사이고 `accessibility-value`를 쓰지 않습니다.
for (const placedOrdinal of [null, 1, 2] as const) {
...
// 창고에서는 자리 번호 래퍼가 렌더되지 않습니다 — placedOrdinal !== null일 때만
// 렌더됩니다.
...
// 0번 조각입니다 — **0은 falsy입니다.** truthy 분기가 들어오면 0번 조각만 조용히
// 죽습니다.
...
// 이미 배치된 조각도 tap을 위로 올립니다 — 배치/해제 판정은 리듀서의 일이지
// 컴포넌트의 일이 아닙니다. 컴포넌트에 두 번째 게이트를 두지 않습니다.
...
// 이 컴포넌트는 상태를 갖지 않습니다 — tap해도 렌더된 data-placed는 그대로입니다.
