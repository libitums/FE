...
// **DOM 순서가 곧 계약입니다** — 낭독 순서 = DOM 순서이므로 시각으로 뒤집지
// 않습니다. 이 화면은 상태를 갖지 않습니다 — 받은 것을 그리고 나가는 수단 하나를
// 냅니다. `criterion`·`verdict`를 prop으로 받지 않습니다 — `assessmentPassCriterion`은
// 자기 모듈에서 직접 읽고, `verdict`는 `results`에서 파생합니다.

// 종합 판정 아이콘입니다. 문항 표식과 같은 어휘(체크/엑스)를 큰 크기
// (`icon-size-xl`)로 재사용합니다.
...
export function AssessmentScreen({
  ...
  // 마운트 때 정확히 한 번입니다. dep 배열이 비어 있습니다 — verdict는 이 화면이
  ...
  return (
    <view className="assessment-screen">
      {/* [고정] 머리 — 제목 하나. 행 상자가 없습니다. back이 없습니다 — 나가는
          수단은 `맵으로` 하나뿐입니다. */}
      <text
      ...
      >
        ...
        <view className="assessment-screen-content">
          <view
            ...
            // 언제나 붙고 값만 갈립니다 — 조건부로 빼지 않습니다.
            data-verdict={verdict}
            // accessibility-label을 붙이지 않습니다 — 안쪽 <text>의 내용이 곧
            // 이름입니다.
          >
            <svg
      ...
      {/* [고정] 액션 행 — 나가는 수단 하나. 판정과 무관하게 라벨이 '맵으로'
          그대로입니다. */}
      <view
