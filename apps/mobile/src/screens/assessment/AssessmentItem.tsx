...
// props 시그니처를 글자 그대로 세운다. 수용 기준 동작(요소 종류 · `data-testid` ·
// `data-result` · `accessibility-*` · 아이콘 · 낱말)은 다음 계층(`ui`)의 몫이다 —
// 여기서 미리 채우면 `ui` 계층의 red가 red가 아니게 된다.
export type AssessmentItemProps = {
  index: number;
  result: ListeningAnswerResult;
};

export function AssessmentItem(_props: AssessmentItemProps): ReactNode {
  return <view className="assessment-item" />;
}
