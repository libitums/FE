...
// 「jest-dom 매처는 절반만 쓴다」). 이 컴포넌트는 상태를 갖지 않습니다 — props에서만
// 파생합니다.
//
// 형태의 정본은 `ListeningChoice.ui.test.tsx`입니다 — `AssessmentItem`은 그
...
// data-result가 언제나 붙고 값만 갈립니다 — "none" 값이 없습니다(평가 행은
// 판정을 반드시 집니다).
...
// 래퍼가 집니다. testid를 늘리지 않아 클래스 셀렉터로 찾습니다 — 찾기는 관찰
// 채널이 아니라 탐색 축입니다(code.md).
for (const { index, result } of RESULTS) {
...
// accessibility-traits가 없습니다 — 이 행은 누를 수 없습니다. button 트레이트가
// 붙으면 없는 조작을 약속하게 됩니다 — 역할 없이 이름만 냅니다.
for (const { index, result } of RESULTS) {
...
// bindtap이 없습니다 — 이 환경에서 `bind*` 이벤트는 `__SetAttribute`가 아니라
// `__AddEvent`를 지나 DOM 속성으로 직렬화되지 않습니다(@lynx-js/testing-environment의
// ElementPAPI가 `bind`로 시작하는 키를 attribute 경로에서 명시적으로 막습니다) —
// 그래서 `bindtap` 자체를 속성으로 질의할 수 없습니다(ListeningScreen.ui.test.tsx
// 단언 15의 같은 관찰입니다). 관찰 가능한 대리 채널은 위의 accessibility-traits
// 부재이고, 여기서는 탭이 아무 것도 하지 않고 조용히 지나간다는 것(=onSelect 같은
// 콜백이 아예 없어 부를 것이 없다는 것)을 회귀로 남깁니다.
for (const { index, result } of RESULTS) {
...
// 이 컴포넌트는 상태를 갖지 않습니다 — 탭해도 렌더된 판정이 그대로입니다.
