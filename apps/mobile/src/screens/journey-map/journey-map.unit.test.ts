...
// 교차 불변식이 보는 조회 함수 셋입니다. 셋 다 순수 로직 모듈이고 DOM·컴포넌트를
// import하지 않으므로 unit 계층의 경계를 넘지 않습니다 (ADR-0006 D4). 듣기 쪽의
// 실제 이름은 `questionsForStep`입니다 (listening.ts:236) — `listeningQuestionsForStep`은
// 뜻을 가리키는 이름이라, 저장소의 이름을 그대로 쓰고 여기서 별칭을 줍니다.
import {
...
// "이 상태가 시트를 여는가"의 판정만 하는 순수 함수입니다. 아무것도 막지
// 않습니다 — 실제 차단(게이트)은 JourneyStepNode.ui.test.tsx가 bindtap 뒤
// onSelect 호출 횟수로 봅니다. 여기서는 반환값만 봅니다.
...
describe("findStep", () => {
  ...
  it("배열에 없는 id면 undefined를 돌려준다", () => {
    // JourneyStepId는 닫힌 union이라, 계약이 없다고 명시한 경우는 as로만 만들 수
    // 있습니다.
...
// 접근성 점검(u8) 지적 ②의 축을 unit이 집니다.
// 지키는 것은 「데이터가 낳는 낭독 이름」이지 렌더 표면이 아닙니다. 표면(속성으로
// 나가는 것)은 JourneyStepNode.ui.test.tsx의 라벨 케이스들이 같은 리터럴로
// 집니다.
// ⚠ 기대값을 journeySteps에서 다시 뽑지 않습니다 — 그러면 동어반복이 되어
// title이 바뀌어도 통과합니다. 리터럴로 못 박는 것이 이 케이스의 전부입니다.
describe("낭독 이름의 데이터 앵커", () => {
  // U-N6
  ...
  // U-N7 — 시트가 읽는 표면입니다. 낭독 이름과 다른 자리라 케이스를 가릅니다.
...
// ------------------------------------------------- 진행 갱신
// 여기부터가 이후 변경이 더하는 것입니다. 위의 케이스는 하나도 지우거나 뜻을
// 바꾸지 않습니다.

// 계약 표의 스텝 다섯입니다. 서수는 journeySteps의 자리(1-based)와 같습니다.
...
describe("journeyStepOrdinal", () => {
  // ("greeting") ("ordering") ("directions") → 1·3·5입니다.
...
describe("completeStep", () => {
  // Math.max(completedCount, journeyStepOrdinal(id))입니다.

  // 수용 기준 8 — 방금 학습한 스텝이 done이 되고 다음이 current가 됩니다.
  it("현재 스텝을 끝내면 진행이 그 스텝의 서수로 는다", () => {
    expect(completeStep(2, "ordering")).toBe(3);
  });

  // 수용 기준 9 — 진행은 되돌아가지 않습니다.
  ...
  // 여정이 전부 완료됩니다 — current인 스텝이 없는 것이 정상입니다.
  ...
  // 「단조성이 계약이다」입니다: 모든 입력에 대해 completeStep(c, id) >= c입니다.
  // 조합으로 단언하지 않으면 분기 하나가 바뀌었을 때 조용히 깨집니다.
...
// 계약의 어휘 넷입니다. **어휘이지 배정이 아닙니다** — 어느 스텝이 어느 값인지는
// 이 파일 어디에서도 단언하지 않습니다.
//
// "culture"가 늘었습니다. journeyStepForStep이 culture를 돌려주는 스텝은 아직
// 없습니다 — 배정은 여전히 열려 있습니다(보류 1b). 이 어휘 배열은 그 배정과
// 무관하게 LearningForm의 네 값을 그대로 나열합니다.
...
// 위한 모듈 내부 표입니다. `Record<LearningForm, …>`이므로 넷째 학습형이 늘면
// 여기가 tsc로 섭니다.
//
// culture: () => 0 — 문화 학습에는 문항이 없습니다. 판정 축이 없고 제시 형태가
// 서사(글) 하나뿐이라 사용자 입력이 0건이고, 채점할 문항 자체가 없습니다.
// 서사는 문항이 아닙니다.
...
describe("learningFormForStep", () => {
  // ⚠ **어느 스텝이 어느 학습형인지를 단언하지 않습니다.** 그것은 배정이고
  // 보류 1b로 아직 열려 있습니다 — 배정을 단언에 박으면 값이 오는 날 계약이
  // 아니라 이 테스트가 배정을 잠급니다. 여기서 보는 것은 「던지는가·undefined인가·
  // LearningForm의 한 값인가」뿐입니다.
...
// 바꾸지 않습니다.
//
// standardUnitSteps는 픽스처로 검사합니다 — 실제 journeyUnits의 구성(유닛이 몇
// 개인지, 어느 스텝이 어느 유닛에 속하는지)은 이음매이므로 이 파일 어디에서도
// 단언하지 않습니다. 아래 픽스처가 쓰는 id는 JourneyStepId가 닫힌 union이라
// 실재하는 값을 빌린 것일 뿐, 그 스텝이 실제로 그 유닛에 속한다는 뜻이
// 아닙니다 — title도 description도 실제 값이 아닌 임의 라벨(A·B·C)을 씁니다.
...
describe("standardUnitSteps", () => {
  ...
  // ⚠ 특별 유닛을 끝이 아니라 사이에 둡니다 — 끝에 두면 slice로도, 첫 유닛만
  // 읽는 구현으로도 통과합니다.
...
// 차가 없습니다. learningFormByStep은 export하지 않으므로, 같은 union으로 타입된
// listeningQuestionsByStep(Record<JourneyStepId, …>)을 그 열거의 대리로 씁니다.
//
// 집합의 차로 짓습니다 — 개수를 세지 않습니다. 길이만 비교하면 「하나 빠지고
// 하나 늘었다」를 통과시킵니다.
describe("journeySteps ⇔ JourneyStepId (파수꾼, U-N5)", () => {
