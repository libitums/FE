// 문화 퀴즈 화면의 순수 로직과 고정 데이터를 소유합니다(ADR-0006 D4 — 순수
// 로직은 unit 계층 대상입니다). 화면 폴더에 있지만 화면 컴포넌트를 참조하지
// 않는 순수 모듈입니다(culture.ts · listening.ts · word-choice.ts와 같은
// 형태) — UI를 import하지 않습니다.
//
// choiceResultAt · optionAccessibilityLabel · hasAnswered는 단어 선택
...
export type CultureQuizQuestion = {
  /** 문항 제시문입니다. 요소로 쪼개지 않습니다. */
  readonly prompt: string;
  /** 보기입니다. 개수를 타입이 말하지 않습니다 — 컨텐츠가 정할 일입니다. */
  readonly choices: readonly string[];
  /** 정답 보기의 인덱스입니다. 범위 불변식은 tsc가 아니라 컨텐츠가 온 뒤의 unit이 집니다. */
  readonly answerIndex: number;
};

/**
 * 세션 상태입니다. 응답 이력을 담지 않습니다 — 판정이 화면 밖으로 나가지
 * 않으므로 이력을 모을 소비자가 없습니다. 죽은 필드를 만들지 않습니다.
 */
...
// ---------------------------------------------------------------- 문항 데이터의 자리
//
// **무엇이 임시인가** — 오른쪽 다섯 값이 빈 것이 임시입니다. 왼쪽 키
// (JourneyStepId 다섯)와 오른쪽 타입(CultureQuizQuestion)은 임시가 아닙니다.
//
// **무엇이 막고 있나** — 문항 컨텐츠는 아직 나오지 않은 페르소나 기획에서
// 오고, culture.ts의 서사와 같은 시점·같은 축에서 옵니다.
//
// **값이 오는 날 무엇만 바뀌나** — 이 표의 오른쪽 다섯 값뿐입니다. 형태도
// export 목록도 화면도 `ui` 테스트도 안 바뀝니다.
//
// **이 표가 배정 근거가 아닙니다** — 다섯 키가 다 있는 것은 `Record`가
// 요구해서이지 다섯 스텝이 전부 문화라는 뜻이 아닙니다. 그리고 문화 퀴즈는
// 스텝에 배정되지 않습니다 — 들어오는 전이는 문화 학습의 액션 행 하나입니다.
//
...
// 총함수입니다. 표(위)가 오늘 다섯 키 모두 빈 배열이라 이 몸통은 이미
// 최종형입니다 — 문항 값이 오는 날에도 이 함수 자체는 안 바뀝니다(표의
// 오른쪽 값만 바뀝니다).
...
// 보기가 correct입니다 — 일치 비교라 0을 falsy로 다루는 자리가 생기지
// 않습니다.
...
// null입니다. 미응답(selectedChoiceIndex === null)은 어떤 choiceIndex와도
// 같지 않으므로 아래 한 줄에 함께 걸립니다. 0번 보기를 골랐을 때도 0 === 0
// 이라 판정이 나옵니다.
...
// 완료도 파생입니다 — 상태에 별도 done 필드를 두지 않습니다.
...
// 전이 넷이 정본입니다 — ① selectChoice + 미응답 → selectedChoiceIndex 갱신
// ② selectChoice + 이미 응답 → 같은 참조(막는 자리가 여기 하나입니다 —
// 컴포넌트에 둘째 게이트를 두지 않습니다) ③ nextQuestion + 미응답 → 같은
// 참조 ④ nextQuestion + 응답 → questionIndex + 1, selectedChoiceIndex: null.
