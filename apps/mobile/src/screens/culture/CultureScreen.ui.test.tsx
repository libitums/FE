...
// 타입만 가져옵니다 — `cultureScreenTitle`·`cultureNarrativeForStep` 같은 값은
// import하지 않습니다. `toHaveClass`·`toHaveStyle`·`toBeVisible`을 쓰지 않습니다
// (docs/conventions/code.md 「jest-dom 매처는 절반만 쓴다」).
//
// 「액션 행이 없다」던 예전 판정의 근거가 소멸했습니다(퀴즈가 서면서 나아갈 곳이
// 생겼습니다). 이 파일 하단의 C1·C2가 그 뒤집힌 판정을 짓습니다 —
// `culture-screen-quiz`가 조건 없이 있고 `맵으로`가 여전히 있습니다(D3).

// fixture 문자열은 culture.ts의 다섯 서사 어떤 값과도 겹치지 않습니다 — 겹치면
// 화면이 실제 데이터에 붙어 있어도 X4·X5가 우연히 통과해 버립니다. 문단이 셋이라
// 실제 표(전부 둘)와 모양이 다릅니다.
...
// 문단 수가 다른 둘째 fixture입니다 — X5의 루프가 fixture에 따라 실제로 갈리는
// 것을 봅니다(공허한 통과 방지). 제목도 첫째 fixture·실제 다섯 서사 어느 것과도
// 겹치지 않습니다.
...
// `onStartQuiz`는 `CultureScreenProps`의 필수 필드입니다(액션 행이 조건 없이
// 렌더되므로 콜백이 비면 조용히 죽은 버튼이 됩니다). 헬퍼 하나로 기본값을 채워
// 넘깁니다 — 예전에는 `onStartQuiz`를 아직 없는 필드로 보고 excess-property
// 우회(변수에 담아 스프레드)를 쓰는 헬퍼가 따로 있었습니다. 그 필드가 실제로
// 필수가 된 뒤에는 우회가 필요 없습니다 — 걷어내고 하나로 합쳤습니다.
...
// 문단 1개짜리 fixture입니다 — 루프가 fixture 크기를 실제로 따라가는지 봅니다.
// 0은 있고 1은 없습니다.
...
// 인자 하나짜리 부정형만 씁니다 — 인자 둘짜리는 다른 값으로 붙어 있어도 통과해
// 부재를 못 짓습니다. culture-screen-scroll 자체는 실재합니다(getByTestId가
// 던지지 않습니다) — 대상이 비어서 통과하는 공허한 통과가 아닙니다.
...
// 조건 없이 렌더된다는 것을 문단이 하나뿐인 fixture에서도 짓습니다 — 내용에 따라
// 조건부로 숨는 자리가 아닙니다.
...
// 액션 행이 `<scroll-view>` **밖**의 화면 직계 자식입니다(ADR-0022 D1). 존재·라벨·
// traits·콜백만으로는 이 구조를 못 잡습니다 — 누가 `culture-screen-quiz`를
// `<scroll-view culture-screen-scroll>` 안으로 옮겨도 C1·C2는 그대로 초록으로
// 남습니다. 그러면 액션 행이 스크롤에 딸려 사라지는데(ADR-0022가 막으려는 바로 그
// 사고) 아무도 못 잡습니다. 형태의 정본은 `CultureQuizScreen.ui.test.tsx`의
// `-scroll` 직계 자식 케이스입니다.
...
// ---------------------------------------------------------------- X9
//
// 「D12-2가 게이트를 통과시킨 자리 하나가 D4의 속성을 실제로 갖는다」를 기계가
// 집니다. 절 제목(`culture-screen-narrative-title`)이 제목 축에 오릅니다.
//
// X1과 합치지 않습니다 — X1은 **화면 제목**을, X9는 **절 제목**을 봅니다. 한
// 케이스로 묶으면 어느 쪽이 실패했는지 러너 출력이 말해 주지 못합니다. 대신 같은
// 케이스 안에서 화면 제목을 **대조**로 함께 단언합니다 — 절 제목에 딸려 화면
// 제목이 사라지지 않는 것을 짓고, 이 둘이 판별 쌍이 됩니다(X9만 빨개지고 X1은
// 초록으로 남아야 합니다).
//
// 부재 단언은 인자 하나짜리 부정형만 씁니다 — 인자 둘짜리는 속성이 다른 값으로
// 붙어 있어도 통과해 부재를 못 짓습니다(같은 파일 X7의 규율입니다). 절 제목은
// 조작 단위가 아니므로(`bindtap`이 없습니다) `accessibility-element`로 묶지
// 않습니다(ADR-0016 D12-3). 공허하지 않음을 짓는 것은 `getByTestId`입니다 —
// 대상이 없으면 거기서 던집니다. 대조 뒤 셋째 `expect`로 오는 `toBeInTheDocument`는
// 그래서 실패할 수 없습니다. 증거가 아니라, 부재 단언의 대상이 실재한다는 것을
// 읽는 사람에게 드러내는 표시입니다.
