...
// 인디케이터)의 크기를 globalProps `safeAreaInsets`로 넘깁니다(`apps/ios`
// ViewController). 앱 셸이 이 값으로 안쪽 여백을 잡아 콘텐츠를 가려지지 않는
// 영역에 두고, 여백 자체는 셸 배경이 칠합니다 — 그래서 화면이 가장자리 색까지
// 정할 수 있습니다(스플래시).
//
// Lynx에는 `env(safe-area-inset-*)`가 없어 값을 CSS가 아니라 JS로 받습니다.
// 값이 없거나(Explorer · 테스트 환경) 모양이 틀리면 0으로 둡니다 — 호스트가 값을
// 넘기기 전 첫 렌더도 0이고, 값이 오면 `useGlobalProps`가 다시 그립니다.
