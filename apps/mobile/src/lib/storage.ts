...
//
// **`lib/audio.ts`는 이 파일과 `typeof` 가드 한 줄이 다르다 — 부주의가 아니다.**
// `ui`·`integration` 테스트 환경에는 `NativeModules` 전역이 **아예 없어서**
// (`hasOwnProperty`가 `false`다) 아래 `nativeModule()` 형태를 그대로 복사하면 맨 식별자
// 접근에서 **`ReferenceError: NativeModules is not defined`** 가 난다. 이 파일은 어떤
// 컴포넌트도 렌더하지 않아 지금까지 드러나지 않았을 뿐이고, `audio.ts`는 **화면이
// 렌더될 때마다** 불린다. **형태를 맞추려고 그쪽 가드를 걷지 않는다.**
//
