...
// 이 파일이 보는 것은 접점의 **페이로드 파서 셋**입니다 — 나머지 export는 전역
// `NativeModules`를 읽으므로 순수 함수가 아니고, 그 경계는 integration 계층이
// 집니다(ADR-0006 D4). 형태의 정본: ./handwriting-recognition.unit.test.ts
//
// 파서는 던지지 않습니다. 모양이 어긋난 값이 오면 예외가 아니라 그 자리의 빈 값이
// 답입니다.
...
// ⭐ 되돌릴 수 있는 거부와 되돌릴 수 없는 제한은 화면이 할 일이 다릅니다(ADR-0026 D3).
...
// 권한 둘이 각각 필드로 남습니다 — 한쪽만 거부된 조합이 실재하므로 뭉치지 않습니다.
...
test("아는 낱말이 아닌 status는 malformed이고 나머지 키는 그대로 읽힌다", () => {
  ...
  // 페이로드 하나가 통째로 버려지지 않습니다 — 볼 수 있었던 것을 스스로 지우지 않습니다.
  expect(result.status).toBe("malformed");
  expect(result.text).toBe("안녕");
  expect(result.durationMs).toBe(700);
});

// ⭐ 빈 문자열은 「듣고도 못 읽었다」의 정식 기록값입니다.
...
test("온디바이스 네 축이 각각 남는다", () => {
  ...
  // 요청했는데 기기가 못 한 자리입니다 — 요청을 안 한 것과 갈려야 합니다.
