...
// 있다. `ui`가 원리적으로 만들 수 없는 트리다(ADR-0016 D12-4의 (나)).
//
// 1) **기존 통합 파일 둘을 0줄로 둔다**. `App.integration.test.tsx` ·
...
//    않는다. 나머지 export는 `importOriginal`로 그대로 통과해 맵·진행이
//    실물로 돈다.
...
// 문항을 순서대로 전부 응답하고 매번 `다음`을 누른다. `App.integration.test.tsx`의
// 동명 헬퍼와 같은 형태다(파일이 다르므로 다시 선언한다) — `listening-complete` 상태에
// 닿는 유일한 수단이다.
...
// 정답/오답을 섞는다 — `App.integration.test.tsx`의 동명 헬퍼와 같은 형태다. 이 파일이
// 묻는 것은 판정값이 아니라 제목 축이므로 섞는 규칙 자체는 결과에 영향을 주지 않는다.
function mixedPick(answerIndex: number, questionIndex: number): number {
  return questionIndex === 1 ? (answerIndex + 1) % 4 : answerIndex;
}

// 제목 축에 오른 자리를 트리 순서대로 앵커로 낸다. `data-testid`가 없는
// 요소는 클래스명으로 찍는다 — 게이트가 배제한 자리 중에는 `data-testid`가 없는 것이
// 있고, `null`로 찍히면 러너 출력이 어느 자리인지 말하지 못한다(`.culture-screen-exit-label`
// · `.assessment-screen-verdict-label`). `getAttribute("class")` 갈래는 발명이 아니다 —
// `ListeningPrompt.ui.test.tsx`가 클래스 목록을 같은 방식으로 대조한 선례를 따른다.
//
// ⚠ (LIB-250, `docs/conventions/code.md` §상태(modifier) 클래스) 이 `class` 갈래는
// 속성 **전체**를 찍는다. 이 저장소는 상태 클래스를 base 클래스 뒤에 **더해** 붙이므로(예:
// `BottomNavigator`의 선택된 탭 라벨 —
// `bottom-navigator-label bottom-navigator-label-selected`), 그런 잎에 `header`가 새면
// 같은 자리가 상태에 따라 서로 다른 문자열로 찍힐 수 있다. 첫 토큰만
// 쓰는 정규화는 **하지 않는다** — 상태 정보가 러너 출력에서 사라지는 대가가 더 크다.
//
// ⚠ (LIB-250, ADR-0016 D2) 셀렉터를 `[accessibility-traits="header"]`
// 에서 `[accessibility-traits]`로 넓히고 값을 쉼표로 갈라 `header`를 포함하는 것만
// 남긴다. 벤더된 Pod(`LynxConverter+UI.m`)의 `toAccessibilityTraits:`가 값을 쉼표로
// split해 OR로 합치므로, `accessibility-traits="button,header"`도 iOS에서 실제로
// 머리말 역할을 싣는다 — 좁은 셀렉터는 그 형태를 못 본다. `D2`가 이 속성에 단일 값을
// 요구하므로 오늘 이 저장소에 쉼표 복수값은 0건이고, 이 갈래는 그 형태가 새로 생겨도
// 잡히게 하는 방어다. 조작 단위 축(`button`·`tab` 단독)은 여전히 안 섞인다 — `header`를
// 포함하지 않으면 필터가 버린다.
...
// ---------------------------------------------------------------- I1 (LIB-243)
//
// ADR-0016 D12-4의 (나) — **결선으로 실제로 연** 문화 화면에서, 실물 서사의 제목이 제목 축에
...
// ---------------------------------------------------------------- I2 (LIB-243)
//
// ADR-0016 D12-4의 (나) — **이 계층의 존재 이유다.** 합성 트리 **전체**를 쓸어 제목 축에 오른
...
// 아니라 나온 것을 비교하는 것이다(ADR-0016 D12-4의 (나)).
//
...
// ---------------------------------------------------------------- I3 (LIB-250)
//
// ADR-0016 D12-4의 (나) — **훑기의 반대 방향.** `[I2]`가 짓는 것은 「이 상태의 트리에 제목
// 축 자리가 이만큼 **있다**」이고, 여기서 짓는 것은 「게이트가 배제한 자리가 이 상태의
// 트리에 **없다**」다. 수단은 같다(`headingAxis` → `toEqual` 닫힌 집합) — 방향만
// 반대다. 게이트가 배제한 자리에 `accessibility-traits="header"`가 붙으면, 그 자리의
// 앵커(테스트가 있으면 `data-testid`, 없으면 클래스명)가 아래 배열에 끼어들어 `toEqual`이
// 깨진다. 상태마다 케이스를 나눈 이유는 ADR-0016 D12-4의 (나)의 탐침이 **자리 하나에만** 속성을
// 붙였을 때 **그 상태를 여는 케이스 하나만** 빨개지는 것을 다음 단계가 이름으로
// 구분해 확인해야 하기 때문이다 — 묶으면 그 구분이 안 된다.
//
// `[I1]`·`[I2]`와 같은 자리를 두 번 안 짓는다: 그 둘은 `culture` 상태 하나만 열고,
// 여기는 그 상태를 다시 열지 않는다 — `culture` 행은 `[I2]`가 이미 진다.
...
// (LIB-250, `docs/conventions/code.md` §쿼리를 무엇으로 고르나 — 존재 앵커) 닫힌 집합
// 대조는 배열이 **자라야** 빨개진다 — 이 상태가
// 무대에 올리는 배제 자리(`.assessment-screen-verdict-label`) 자신이 트리에서
// 없어지면 배열은 안 자라고 아래 `toEqual`은 계속 통과한다. `assessment-screen-title`
...
// (LIB-250, `docs/conventions/code.md` §쿼리를 무엇으로 고르나 — 존재 앵커; ADR-0016 D4
// 정정 — 개수는 문면에 박지 않는다) 아래 케이스들(`word-choice`·`sentence-order`·
// `culture-quiz`)은 각 화면의 문항 표가 오늘 비어 있어 **종료 상태**를 연다. 배제
// 자리(`*-screen-complete`)가 곧 종료 상태의 프로브이므로, 그 자리 자신을
// `getByTestId`로 먼저 짓는다(이들은 클래스명과 같은 이름의 `data-testid`를 가진다) —
// 화면 제목만으로는 이 자리의 존재를 아무도 안 진다.
//
// 문항 표가 채워지는 날 이 케이스들은 조용히 **문항 상태**를 열게 된다(도달 절차만
// 낡는다) — 그날 이 앵커(`getByTestId`)가 문항 상태의 트리에서 그 자리를 못
...
// ------------------------------------------------- (LIB-250, ADR-0016 D12 게이트;
// ADR-0016 D12-4의 (나) — 훑기의 반대 방향)
//
// 위 케이스들이 여는 상태 전부는 학습 세션 안이다. 사용자가 실제로 먼저 닿는
// 상태들 — 네 탭 뿌리, 시트 열림, 듣기의 문항 상태(가장 오래 머무는 상태) — 은
// ADR-0016 D12-4의 (나)에 앵커는 올라 있지만 어느 `toEqual`에도 들어온 적이 없었다. 그 상태에
...
// 시트 열림 — `journey-step-node-<stepId>` tap 직후, `시작`은 아직 안 눌렀다. 맵
// 제목과 시트 제목이 같은 트리에 함께 선다(맵 컨테이너는 `accessibility-elements-hidden`
...
// 듣기의 문항 상태 — `startStep`으로 세션에 들어간 직후, 아직 한 문항도 안 풀었다
// (`answerAllQuestions`를 부르지 않는다). 오늘 사용자가 실제로 닿는 유일한 학습
