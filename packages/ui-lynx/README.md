...
`Card`, `ChatBubble`, `VisualNovelDialog`, `TextField`, `AnswerLabel`, `Overlay` 열여섯 가지다.
...
<VisualNovelDialog variant="speech" speakerName="아리아" line="달빛 아래에서 다시 만났네." />;
```
...
규칙을 따른다. 공개 컴포넌트 열여섯 개 모두 `<component>.contract.ts`에 공개
...
검증한다. package integration test도 이 부재 계약을 열여섯 subpath 전체에서 확인한다.
