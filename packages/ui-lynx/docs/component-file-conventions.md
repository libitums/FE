...
현재 공개 컴포넌트 열두 개(`Button`, `BackHeader`, `StatusIndicator`, `RoundButton`,
`ProgressHeader`, `PageIndicator`, `BottomNavigator`, `StepIndicator`, `BottomSheet`,
`CompactNumericInput`, `ChatBubble`, `TextField`)가 모두 이 구조를
사용한다. 순수 runtime export가 없는 `BackHeader`만 빈 unit test를 두지 않고, 나머지
열한 개는 PascalCase unit/UI test 쌍을 둔다.
