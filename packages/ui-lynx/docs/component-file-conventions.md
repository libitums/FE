...
현재 공개 컴포넌트 열여덟 개(`Button`, `BackHeader`, `StatusIndicator`, `RoundButton`,
...
`Overlay`, `Fog`, `Tooltip`)가 모두 이 구조를
사용한다. 순수 runtime export가 없는 `BackHeader`만 빈 unit test를 두지 않고, 나머지
열일곱 개는 PascalCase unit/UI test 쌍을 둔다.
