...
**단일 회차이고 설치 방식이 앞 넷과 다르다.** 두 변수가 한 점에 겹쳐 있어 **어느 것이 45 ms를 냈는지 판정할 수 없다.**

**그리고 첫 측정을 버린 것 자체가 이 회차의 신뢰도를 낮춘다** — 무엇이 그 거짓 출력을 냈는지 못 갈랐으므로, **같은 원인이 이 두 번째 측정에 없다는 근거가 없다.** `paintEnd`가 다르다는 것은 「같은 파일을 두 번 읽지 않았다」를 보일 뿐이다.

**시간 수치를 다음 회차의 기준선으로 쓰지 마라.** 메모리 다섯 값과 번들 크기만 대조에 쓸 수 있다 — 그 둘은 결정적이고 재현된다.
