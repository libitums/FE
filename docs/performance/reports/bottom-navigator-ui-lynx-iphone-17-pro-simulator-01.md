...
측정 뒤에 치수와 배치를 더 고쳤습니다 — 바 세로 패딩 24 → 4px, 아이콘 상자 32 → 24px,
바가 설 때 셸이 두는 아래 여백을 홈 인디케이터 높이 대신 12px로, 그리고 바를 흐름에서 빼
콘텐츠 **위에 겹치도록**(절대 배치) 옮기고 화면 열여섯 곳에 그만큼 하단 여백을 더했습니다.
**이 회차는 그것들을 반영하기 전 빌드입니다.**

앞의 셋은 CSS 치수만 바꾸므로 element 수와 update 경로가 같지만, 마지막 것은 다릅니다 —
바가 절대 배치로 바뀌면 레이아웃 계산이 달라질 수 있고, 전환 pipeline의 `layout` 몫이 그
영향을 받는 자리입니다. **여기 숫자를 그 뒤 코드의 값으로 읽으면 안 됩니다.**
