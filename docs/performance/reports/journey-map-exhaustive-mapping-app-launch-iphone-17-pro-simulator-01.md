...
`switch`로 바꾸고 `default`를 두지 않았습니다. 갈래가 빠지면 반환 타입에 `undefined`가 섞여
컴파일이 섭니다 — 화면 타입을 하나 더해 실제로 서는 것을 확인한 뒤 되돌렸습니다.
...
`mainThreadRuntimeBytes`도 1,271,744 ~ 1,286,208 bytes로 14 KB 폭 안에서 오르내립니다. 앞선
보고서에서 13 KB 차이를 두고 「늘었다」고 적은 것이 이 폭 안이라는 뜻입니다.
...
모이면서 이 시나리오의 흔들림 폭이 초기 `pipeline` 약 11 ms, 메인 스레드 메모리 약 14 KB로
관측됐다는 것까지가 이 기록이 말할 수 있는 전부입니다. 그 폭보다 작은 차이는 앞으로도
해석하지 않습니다.
