...
같은 commit을 다섯 번 재 보니 `mainThreadRuntimeBytes`는 **전혀 흔들리지 않습니다**. 그러므로
위 13 KB는 측정 잡음이 아니라 코드 변화에서 온 실제 차이입니다. 렌더 시간 쪽 서술(1.6 ms 차이를
해석하지 않는다)은 그대로 유효합니다 — 그 축의 흔들림 폭은 12.6 ms입니다.
