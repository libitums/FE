...
**변수 이름은 `@libitums/design-tokens`가 정의한 것만 쓴다.** 접두사는 예외 없이
`--libitum-`이고, **다른 이름 계열을 저장소 안에서 만들지 않는다.** 패키지에 없는 이름이
필요하면 새 이름을 지어내지 않고 D4를 따른다 — design-system에 요청한다.

> 이 문단은 원래 위 예시로만 보였다. **문장으로 적히지 않아 다른 계열이 통과했다**
> (아래 `정정 기록` 2026-09-02).
...
### D8. 토큰 이름 검사를 `lint`에 붙인다 — 지금은 **접두사 검사만**

ADR-0011의 재검토 조건 *"패키지가 배포되면 알려진 토큰 이름만 허용하는 CSS 검사를
`lint`에 붙인다"* 는 **조건을 충족했다.** 전부는 지금 붙일 수 없으므로 절반을 붙인다.

| 무엇을 보나 | 지금 | 왜 |
|---|---|---|
| CSS의 `var(--…)`가 `--libitum-` 접두사인가 | **붙인다** | 저장소 파일만 보면 판정된다 |
| 그 이름이 패키지에 **실제로 있는가** | **보류** | 배포된 `variables.css`의 변수 목록과 대조해야 한다 (`docs/adr/README.md` 보류 표) |

- 대상은 `apps/mobile/src/**/*.css`. 어긋난 줄이 하나라도 있으면 `pnpm lint`가 실패한다.
- `oxlint`는 CSS를 보지 않는다. 검사는 `package.json`의 script 한 줄로 둔다 —
  **스크립트 디렉터리를 만들지 않는다**(ADR-0003 D4). 이 검사를 넣는 PR에서 ADR-0006 D1의
  `lint` 행 서술(*"정적 검사(`oxlint`)"*)도 같은 PR에서 함께 고친다 (ADR-0010 D4).
- **잡히는 것**: 이름 계열이 갈라지는 것. 실제로 일어난 실패가 그것이다.
- **안 잡히는 것**: 접두사가 맞는 오타(`--libitum-colour-…`). 보류 항목이 닫혀야 잡힌다.
  그때까지 `docs/e2e/design-token-rendering.md`의 눈 확인이 유일한 수단이고,
  **D1이 감수한 실패의 절반은 그대로 남는다.**

**근거는 이 실패가 조용하다는 것만이 아니다.** 선언이 통째로 버려지고 에러가 없으며
테스트가 계산된 스타일을 못 보는 것(ADR-0006 D4)까지는 알고 감수한 것이었다. 새로 안
것은 **사람이 diff를 읽는 게이트(ADR-0009 D4)도 이것을 못 잡는다**는 것이다 —
...
- **접두사 검사도 넣지 않고 PR diff에서 본다** — 이 저장소의 기본 태도이고
  (`ui` 테스트 동반·jest-dom 매처 선택이 그렇다), 도구를 안 늘린다. 버린 이유는 바로
  위다 — **그 게이트가 이미 한 번 놓쳤고, 놓친 이유가 게이트의 구조**다 (D8).
- **알려진 이름 전체 대조를 지금 만든다** — 오타까지 잡는다. 그러나 `node_modules`가
  없으면 대조할 목록이 없어 검사 자체가 환경에 따라 통과/실패가 갈린다. **설치 상태에
  의존하는 검사는 `lint`에 둘 수 없다.** 보류 표로 넘긴다 (D8).
...
- **토큰 이름 오타를 절반만 자동으로 잡는다.** D8의 접두사 검사가 이름 계열이 갈라지는
  것까지만 잡고, 접두사가 맞는 오타는 그대로 남는다. ADR-0011 D1이 감수한 실패의 절반이다.
- **검사가 하나 더 늘었다.** `lint`가 `oxlint` 하나가 아니게 되고, CSS 검사가 오탐을
  내면 그것을 우리가 고쳐야 한다 (D8).
...
**2026-09-02 — D1의 문구가 다른 이름 계열을 허용했다.** 결정(CSS 커스텀 프로퍼티로
소비)은 그대로다.

**어떻게 틀렸나** — **변수 이름을 예시 코드로만 보이고 문장으로 적지 않았다.**
그래서 `apps/mobile/src/app/error-boundary.css`가 `var(--color-bg-surface)` ·
`var(--spacing-16)`를 쓰는 채로 남았는데도 **D1의 어느 문장도 어기지 않았다.**
골격 커밋(#13)이 쓴 이름 계열이고, 패키지를 실제로 붙인 커밋(#28)이 홈 화면만 갱신하고
지나갔다. **이 자리는 한 번 적혔던 자리다** — ADR-0013의 맥락(2026-08-26)이
*"`var(--color-bg-surface)` · `var(--spacing-16)`을 쓰는데 그 값을 공급하는 것이 저장소
어디에도 없다"* 고 적어뒀다. **공급자가 생긴 날 그 문단은 닫힌 것으로 읽혔고, 파일 하나가
남은 것은 아무도 다시 세지 않았다.** 한 저장소 안에 이름 계열이 둘 있게 됐고, **어느 쪽이 맞는지는 이 ADR을 열어야만
알 수 있었다.** 규약 문서는 오히려 틀린 쪽을 예시로 들고 있었다.

ADR-0010 D10의 *"실체화가 결정을 만족하지 않으면 파일을 고친다. 다만 문구가 그 어긋남을
허용했다면 문구를 조이고 `정정 기록`"* 에 정확히 해당한다. **셋을 다 한다** — 문구를
조였고(D1), 재발을 잡을 검사를 넣었고(D8), 파일은 다음 실체화에서 고친다.
`--color-bg-surface`·`--spacing-16`에 대응하는 이름은 **그때 배포된 `variables.css`에서
고른다.** 없으면 지어내지 않고 D4를 따른다.

> **예시는 규칙이 아니다.** 예시가 규칙을 보여준다고 믿으면, 예시를 안 본 사람은 규칙을
> 안 어긴 것이 된다. 이 ADR에서 값·이름·경로를 예시로만 보인 자리가 여기 하나뿐인지
> 확인했다 — `variables.css`·`typography.css`의 import 경로가 같은 모양이지만
> 그쪽은 **문장이 "진입점에서 한 번만"을 이미 적고 있다.**

**같은 날 — 규약 문서가 이 ADR 이전 상태에 멈춰 있었다.** `docs/conventions/code.md`의
"스타일" 절이 예시로 `var(--color-bg-surface)`를 들고, **TS 상수를 전면 금지**하며,
대체된 ADR-0011을 링크한다. 셋 다 이 ADR과 어긋난다 — D1의 이름 계열, D2가 연
아이콘 색 예외, 링크. 이 ADR을 채택한 PR(#28)이 규약 문서를 쓸어보지 않았다.
**ADR-0010 D4를 어긴 것이 아니라 D4의 문구가 이 경우를 덮지 않았다** — 그쪽 문구도
함께 조였다 (ADR-0010 D4 · `정정 기록` 2026-09-02).

## 재검토 조건

- **중첩 `var()`가 안 되는 원인** → 규명되지 않았다. 엔진은 지원하므로 빌드/encode 경로를
  봐야 한다. 밝혀지면 D1의 평탄화 의존을 다시 본다. upstream 보고 대상이기도 하다
- ~~**토큰 이름 검사를 붙일 수 있게 됐다**~~ → **절반 닫혔다.** 접두사 검사는 D8이
  가져갔다. 남은 것은 **알려진 이름 전체 대조**이고, `variables.css`의 변수 목록을
  대조 대상으로 고정할 수 있게 되면 붙인다 (`docs/adr/README.md` 보류 표).
  **`var(--libitum-오타)`가 조용히 무시되는 것을 잡을 유일한 수단이다** —
  D1이 감수한 실패의 남은 절반이다
