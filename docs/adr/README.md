...
| **Dynamic Type 배율의 상한** | 무엇이 깨지는지 아직 안 봤다 | ADR-0020 D4가 **상한 없이 넣고 관측하기로** 했다. Apple의 접근성 크기는 `.body` 기준 3배를 넘는데, 상한을 먼저 걸면 **깨지는 자리가 가려지고 가려진 것은 고칠 대상이 되지 못한다.** 상한은 레이아웃을 고칠 수 없다고 판정된 뒤의 수단이지 그 앞의 수단이 아니다. **이 행은 실기 훑기 전에는 닫히지 않는다** | **본인** | 최대 배율에서 세 화면을 훑고 깨지는 자리 목록이 나올 때 (LIB-224) |
