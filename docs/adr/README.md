...
| **선택 상태 표현 형태 확정** (`accessibility-value` vs 라벨 접미사) | 낭독을 들어야 정할 수 있다 | 타입으로는 `accessibility-traits`가 단일 값이라는 것까지만 확정됐다(ADR-0016 D2). 어느 쪽이 VoiceOver에서 자연스럽게 읽히는지는 **실기 낭독**이 유일한 판정 수단이다. 지금은 `value`로 정해 두었고 뒤집힐 수 있다 (ADR-0016 D3) | **본인** | 자체 호스트 iOS 실기 — `docs/e2e/tab-navigation.md` 6번 |
