...
| 대가 | 이 선택으로 포기하는 것 |
| 정정 기록 | 사실 오류를 제자리에서 고쳤을 때만 붙인다. 날짜와 **어떻게 틀렸는지** |
...
| `@libitum/*` 버전 범위 | SemVer·changelog 정책이 아직 없다 | LIB-128 미확정 (ADR-0011). **그때까지 첫 설치는 정확 버전으로 적어둔다** — `pnpm add`의 기본값(caret)이 조용히 규칙이 되는 것을 막는 잠정 조치다 | **본인** — 다른 저장소의 내 대기열이다 | LIB-128 확정 후 |
