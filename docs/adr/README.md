...
| **문화 서사 제목의 `header` 트레이트** (내용 구획 제목을 제목으로 낼 것인가) | **갈래가 이 저장소에 처음 생겼다.** `culture-screen-narrative-title`은 시각으로는 제목이다 — 화면 제목과 **같은 typography scale(`heading-s`)·같은 색(`fg-neutral`)** 인데 보조기술 채널이 **0건**이다. **WCAG 1.3.1 Info and Relationships(Level A)** 가 「시각적 표현으로 전달되는 구조는 프로그램적으로 결정될 수 있어야 한다」를 요구하는 자리인데, **ADR-0016 D4의 열거는 「화면·시트 제목」까지**이고 **「내용 구획 제목」은 그 열거에 없다.** D4가 「화면당 header 하나」를 금지한 적도 없다 — **선례의 부재는 금지가 아니다.** 그래서 못 정하는 것은 「붙일 것인가」가 아니라 **「어느 결정이 이 자리를 덮는가」** 다 | **ADR-0016을 먼저 여는 판단.** D4의 열거를 「화면 제목」에서 **「제목을 지는 자리」** 로 넓히거나 「내용 구획 제목」을 새 결정으로 연다. **D4 자신의 근거가 화면 축이 아니다** — *"제목을 제목으로 내는 방식이 자리마다 갈리면 안 된다"* 라고 적었고, 여섯째(스텝 시트)가 **화면이 아닌데도** 같은 행에 들어오면서 *"열거가 늘어난 것이지 결정이 바뀐 것이 아니다"* 로 처리된 선례가 있다. **⚠ 그래서 LIB-238이 일부러 안 고쳤다** — 화면 CSS·TSX만 고치면 저장소에 **근거 없는 자리가 하나** 생기고(heading scale을 입은 텍스트 열여섯 중 `header`가 있는 것은 화면·시트 제목뿐이다), 다음 사람은 그것이 결정인지 사고인지 구분할 수 없다. **이 행이 없으면 다음 감사가 이 논증을 처음부터 다시 한다** | **본인** | **실기가 숫자를 낸 뒤.** `docs/e2e/culture.md` **M5**가 로터 「제목」 순회의 정지 수와 **긴 서사에서 머리→본문 시작 스와이프 횟수**를 잰다 — 그 수가 ADR-0016을 여는 근거의 크기다 |
