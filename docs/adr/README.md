...
세 케이스를 확인했다 — 정의된 `var()`는 적용되고, `var(--x, 기본값)`의 fallback도 동작하며,
**fallback 없는 미정의 변수는 아무 색도 내지 않고 조용히 무시된다.** 마지막 것이 ADR-0011 D1이
*"알고 감수한다"* 고 적은 실패이고, **그렇게 동작하는 것을 눈으로 확인했다.**
