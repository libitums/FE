...
`scroll-view` 사용이 저장소 전체에 **0건**이다. 화면은 전부 고정 뷰포트의 flex 열이고,
...
요구하는 것은 200%**이고, 2.0은 그 요구를 정확히 채운다.

**이 상한은 사용자 설정을 거부하는 것이 아니다.** 지금은 3.1배에서 **앱을 못 쓴다.**
2.0으로 막으면 **못 쓰던 것이 쓸 수 있게 된다.** 상한을 없애는 것은 스크롤이 닿은
뒤에 할 일이다.
...
- **`scroll-view`가 들어오면 상한을 걷는다.** 그것이 상한을 둔 유일한 이유다.
  걷을 때 최대 배율(약 3.1배)에서 세 화면을 다시 훑는다
