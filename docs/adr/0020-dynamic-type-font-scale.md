...
> **이 결정이 덮지 않는 축 하나 (2026-09-05, LIB-228).** **한 flex 상자 안에서 배율을
> 받는 자식과 안 받는 자식의 크기 비가 갈릴 때 무엇을 거는가**는
> **[ADR-0023](0023-scale-mismatch-in-flex-boxes.md)이 맡는다.** D5는 **한 상자가 잘리는
> 것**을 덮고, 0023은 **상자 안 자식들 사이의 불일치**를 덮는다 — 가로 교차축에서 그것이
> 내는 것은 잘림이 아니라 **낭독 순서 역전**이라 WCAG 1.4.4가 아니다. **이 결정은 안
