...
- ⚠ **로그인 화면은 상태를 하나도 들지 않는다 — 계약 문면과 갈리는 자리이고 사람 결정을 기다린다.**
  화면이 `TextField`에 `bindinput`을 넘기지 않아 **전화번호 입력값이 어디에도 남지 않는다**
  (화면에 `useState` 0건). 계약 §2.6의 문면은 이 행에 `bindinput`이 **있다**고 적었고
  `availability`는 **넘기지 않는다**고 적었는데 **실물은 양쪽 다 반대**다 — 넘기는
  `availability="enabled"`가 기본값과 같아 **동작·속성 델타는 0**이다. 계약을 실물에 맞추면
  **공개 계약 변경**이라 이 단위가 정하지 않았다: **LIB-261 계약 §15 Q-6**으로 올라가 있다.
