...
**내용 영역**에만 들어간다(구조 계약 §1.1). 다만 이 흐름이 홈·여정·롤플레이·설정
네 화면을 모두 지나가므로, 그 화면들에 새로 생긴 것 둘을 여기서 함께 본다. 최대
...
**S11. 홈·롤플레이·설정의 빈 스크롤 영역이 정지를 하나 늘리는가**
- 전제: VoiceOver를 켠 상태
- 단계: **0단계(기준선).** 이 빌드에서 세기 **전에**, 스크롤 컨테이너가 없는 상태의
  정지 수를 먼저 센다. 그 빌드를 만들 수 없으면 **세 화면의 정지 수를 먼저 적고**,
  「늘었는가」가 아니라 **「무엇이 정지인가」의 목록**을 적는다. 그다음 홈 → 롤플레이 →
  설정 순으로 각 화면을 VoiceOver로 훑어 정지 횟수를 센다
- 관찰: **적는다.** 세 화면은 지금 정지가 제목 하나라, 늘면 **100% 증가**여서 가장
  잘 보이는 자리다 — `listening.md`의 **E-S4**를 내용 없는 화면에서 다시 보는 것이다.
  홈의 아이콘 `<svg>`가 정지로 잡히는지는 **모른다** — 붙어 있는
  `accessibility-elements-hidden`은 **자손이 없어 무동작**이고(ADR-0016 D9), 잎이
  정지가 되는지는 이 저장소에서 확인된 적이 없다
- 어긋나면(= 정지가 늘면, R6): **여섯 곳 전부**에 같은 속성으로 대응한다 —
  다섯에 **평가 화면(LIB-227)이 여섯째로 붙었다.** 세 화면만 고치지 않는다
  (요구사항 결정 1). **결론은 안 바뀐다** — 개수만 는다

**S14. 최대 배율에서 홈·롤플레이·설정의 고정 머리가 눌리는가**
- 근거: 구조 계약 §6.1이 design에 되돌려 물은 질문 2("고정 슬롯의 축소 방지")가
  **열려 있다.** `flex-shrink: 0`이 지금 듣기 화면의 셋(`-header`·`-next`·`-finish`)
  에만 있고, `.home-screen-header`·`.roleplay-list-screen-title`·
  `.settings-screen-title`에는 없다. 계약도 design도 값을 정하지 않아 구현이
  손대지 않았다 — **`journey-map.md`의 S13이 같은 질문을 여정 맵에서 먼저 본다.**
  이 항목은 그 질문을 세 화면에서 다시 본다
- 단계: 최대 배율(`listening.md`의 「`FontScale.cap` 제거 절차」를 먼저 마친 뒤)에서
  홈 → 롤플레이 → 설정을 차례로 연다. 각 화면 머리의 세로 치수를 잰다
- 관찰: 셋 다 머리가 배율만큼 자라 있고(글자가 잘리지 않는다), **눌려서 원래
  높이보다 작아지지 않는다.** 흐르는 영역은 비어 있어 넘칠 내용이 없지만, 머리
  자신의 글자가 커지면서 전체 높이가 눌릴 자리가 있는지를 본다
- 어긋나면: 각 화면 머리 클래스에 `flex-shrink: 0`을 추가한다. **여섯 화면 전부
  같은 보장**을 갖는 것이 계약 §6.1의 요구이므로 화면별로 다르게 고치지 않는다 —
  `journey-map.md`의 **S13**, `assessment.md`의 **E5**(`.assessment-screen-title`)가
  어긋난 경우와 **함께** 판단한다. 평가 화면(LIB-227)이 같은 질문을 받는 여섯째다

**S16. 내용이 0개인 흐르는 영역이 높이 0으로 접히는가**
(design.md §6.5 ⟨실기 판정 지점⟩ · §7 가정 4)
- 근거: 홈·롤플레이·설정의 흐르는 영역은 **자식이 0개**다(`HomeScreen.tsx:35` ·
  `RoleplayListScreen.tsx:21` · `SettingsScreen.tsx:21`). design은 그것이 `flex: 1`로
  남는 높이를 그대로 차지한다고 가정했고(§7 가정 4), 어긋나면 이렇게 된다 —
  *"내용이 0개인 `<scroll-view>`가 높이 0으로 접히면 선이 제목에 달라붙어 보인다"*
  (design.md §6.5). 세 화면에 **새로 생긴** hairline이 「머리와 내용을 가르는 선」이
  아니라 제목의 밑줄로 읽히는 것이다
...
- 단계: 홈 → 롤플레이 → 설정을 차례로 연다. 각 화면에서 **hairline 위쪽 모서리부터
  바텀 네비게이션 위쪽 경계까지의 거리**를 재고, 거기서 `screen-padding-bottom` **24**를
  뺀다 — 그것이 흐르는 영역이 실제로 차지한 높이다
- 관찰: **숫자를 적는다.** 셋 다 그 높이가 design.md §6.4가 낸 값(iPhone 13 mini 기준
