...
**S11. 홈·롤플레이·설정의 스크롤 영역이 정지를 하나 늘리는가**
- 전제: VoiceOver를 켠 상태
...
- 관찰: **적는다.** 홈·설정은 지금 정지가 제목 하나라, 늘면 **100% 증가**여서 가장
  잘 보이는 자리다 — `listening.md`의 **E-S4**를 내용 없는 화면에서 다시 보는 것이다.
  **롤플레이는 제목 하나가 아니다(LIB-255)** — 기대 정지는 제목 + 항목 셋(항목마다 하나,
...
- 관찰: 셋 다 머리가 배율만큼 자라 있고(글자가 잘리지 않는다), **눌려서 원래
  높이보다 작아지지 않는다.** 홈·설정의 흐르는 영역은 비어 있어 넘칠 내용이 없고,
  롤플레이는 항목 셋이 흐르는 영역 안에서만 스크롤된다(LIB-255). 어느 쪽이든 머리
...
- 근거: 홈·설정의 흐르는 영역은 **자식이 0개**다(`HomeScreen.tsx:35` ·
  `SettingsScreen.tsx:21`). 롤플레이는 LIB-255부터 목록 상자 하나를 품어 이 항목에서
  빠졌다(위 전제 정정). design은 그것이 `flex: 1`로
  남는 높이를 그대로 차지한다고 가정했고(§7 가정 4), 어긋나면 이렇게 된다 —
  *"내용이 0개인 `<scroll-view>`가 높이 0으로 접히면 선이 제목에 달라붙어 보인다"*
  (design.md §6.5). 그 화면들에 **새로 생긴** hairline이 「머리와 내용을 가르는 선」이
  아니라 제목의 밑줄로 읽히는 것이다
...
- 단계: 홈 → 설정을 차례로 연다. 각 화면에서 **hairline 위쪽 모서리부터
  바텀 네비게이션 위쪽 경계까지의 거리**를 재고, 거기서 `screen-padding-bottom` **24**를
  뺀다 — 그것이 흐르는 영역이 실제로 차지한 높이다
- 관찰: **숫자를 적는다.** 둘 다 그 높이가 design.md §6.4가 낸 값(iPhone 13 mini 기준
