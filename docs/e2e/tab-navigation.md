...
전제: 공통 전제와 같다 (자체 호스트 앱). 첫 화면은 여정 맵이고 바텀 탭은 이미 "여정"이
선택된 채로 시작한다(LIB-257로 홈 탭이 걷혔다)
...
- 관찰: **적는다.** 설정은 지금 정지가 제목 하나라, 늘면 **100% 증가**여서 가장
...
- 관찰: 둘 다 머리가 배율만큼 자라 있고(글자가 잘리지 않는다), **눌려서 원래
  높이보다 작아지지 않는다.** 설정의 흐르는 영역은 비어 있어 넘칠 내용이 없고,
...
(design.md §6.5 ⟨실기 판정 지점⟩ · §7 가정 4. LIB-257로 홈이 걷혀 대상은 설정
하나가 남는다)
- 근거: 설정의 흐르는 영역은 **자식이 0개**다(`SettingsScreen.tsx:21`). 롤플레이는
  LIB-255부터 목록 상자 하나를 품어 이 항목에서 빠졌다(위 전제 정정). design은
  그것이 `flex: 1`로 남는 높이를 그대로 차지한다고 가정했고(§7 가정 4), 어긋나면
  이렇게 된다 — *"내용이 0개인 `<scroll-view>`가 높이 0으로 접히면 선이 제목에
  달라붙어 보인다"* (design.md §6.5). 그 화면에 **새로 생긴** hairline이 「머리와
  내용을 가르는 선」이 아니라 제목의 밑줄로 읽히는 것이다
