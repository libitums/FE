...
- 전제: VoiceOver를 켠 상태
- 단계: 홈 → 롤플레이 → 설정 순으로 각 화면을 VoiceOver로 훑어 정지 횟수를 센다
- 관찰: **적는다.** 세 화면은 지금 정지가 제목 하나(홈은 아이콘이 가려져 있어
  역시 하나)라, 늘면 **100% 증가**여서 가장 잘 보이는 자리다 — `listening.md`의
  **E-S4**를 내용 없는 화면에서 다시 보는 것이다
