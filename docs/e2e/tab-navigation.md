...
확인되지 않았는지 셀 수 없다. **열두 개 중 열을 판정했다 — 아홉 통과, 하나 실패.**
둘(8 · 10)은 아직 보지 않았다.
...
> **수용 기준 8은 실기에서 실패했다** — 선택 상태가 낭독되지 않는다(6번). 이름·역할·
> 묶음·제목은 통과했다(5 · 7 · 9). **수용 기준 9는 아직 낭독 미확인이다**(10번을 보지
> 않았다). `ui`가 전부 green인 것은 속성이 붙었다는 뜻이지 보조기술이 그것을 읽는다는
> 뜻이 아니라는 것을, 6번이 실물로 보여줬다.
>
...
| **6** | ❌ **실패** → 재판정 대기 | **"선택됨"이 들리지 않는다.** 선택된 탭도 `"홈, 버튼"` 이다. 이 결과로 결정이 바뀌었다 (아래) |
...
발동했다.** D3이 남긴 대안 둘 중 **대칭 두 문구(`"선택됨"` / `"선택 안 됨"`)도 함께
