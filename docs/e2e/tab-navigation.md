...
### 판정하지 못함 — 5 ~ 10 (VoiceOver 낭독)

**시뮬레이터에서 VoiceOver를 켜지 못했다.** `VoiceOverTouchEnabled=1`을 쓰고
`notifyutil`로 변경을 알렸지만 커서도 캡션 패널도 뜨지 않았고, 화면 차이는 시계뿐이었다.
최근 Xcode의 Simulator는 VoiceOver를 지원하지 않는다.

**이 여섯은 실기기 또는 Accessibility Inspector가 필요하다.** 속성이 붙은 것은
`ui` 계층이 `toHaveAttribute`로 검증했지만, **iOS가 그것을 어떻게 읽는지는 여전히
아무도 보지 않았다.** 수용 기준 8·9는 그대로 "속성까지 고정·검증됨, 낭독 미확인"이다.

여섯 중 판정이 뒤집힐 수 있는 것 둘을 다시 짚는다 — **9번은 대안이 없고**(iOS에서
`accessibility-heading`을 쓸 수 없다), **6번이 뒤집히면 ADR-0016 D3을 재고정**한다.
