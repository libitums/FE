...
- 근거: 구조 계약 §6.1이 design에 되돌려 물은 질문 2("고정 슬롯의 축소 방지")가
  **열려 있었다.** **LIB-228(`f354612`)이 `.home-screen-header` ·
  `.roleplay-list-screen-title` · `.settings-screen-title` 셋에 `flex-shrink: 0`을
  걸었다** — FE ADR-0023 규칙 2의 사례이고, `journey-map.md`의 **S13**이 같은 질문을
  여정 맵(`.journey-map-screen-title`)에서 먼저 본다. 이 항목은 그 질문을 세
  화면에서 다시 본다
