...
[고정] 액션 행   .culture-quiz-screen-next                            flex-shrink: 0
```
...
| 판정 뒤 | `다음` (`-next`) |
| 완료 | **아무것도 없다.** 액션 행 자체가 서지 않는다 — 출구는 머리의 `맵으로` 하나다 |

**영구 `disabled` 버튼을 두지 않는다** — 「다음」은 *아직* 불가이지 *영구히* 불가가 아니다
(ADR-0016 D10). 존재 자체가 상태이므로 속성을 또 붙이지 않는다.

> ~~**완료 뒤 출구의 정체는 계약이 정한다.**~~ **닫혔다 (2026-09-07, LIB-244 D1).**
> **출구는 머리의 `맵으로` 하나이고 `-finish`는 서지 않는다** — 계약이 *"퀴즈 판정은 진행을
> 걸지 않는다"* 로 정해 **결과를 넘길 곳이 없어졌고**, 그래서 `결과 보기`에 해당하는 조작
> 단위가 이 화면에 **0건**이다. 예고한 대로 `-finish` 블록을 **지웠다.**
> ⚠ **`docs/screens.md`의 미정 「진행에 거는 방식」은 이것으로 닫히지 않는다** — 그 미정이
> 묻는 것은 「문화 스텝이 스텝 하나인가 스텝 안의 한 단계인가」이고 **퀴즈 판정의 유무와
> 독립**이다. 여기서 닫힌 것은 **오늘 이 화면의 출구** 하나다.

**나가는 수단은 어느 시점에도 정확히 하나다**(ADR-0022 D1). 머리의 `맵으로`와 액션 행의
`다음`은 충돌하지 않는다 — 후자는 나아가는 수단이다. ~~`-finish`가 서는 완료 시점에는 머리의
`맵으로`가 빠진다(`WordChoiceScreen`이 이미 그 형태다).~~ **그 전제가 사라져 적용되지 않는다** —
`-finish`가 안 서므로 `맵으로`를 대신할 것이 없고, 걷으면 완료 상태에서 **나가는 수단이 0건**이
되어 ADR-0007 D3이 깨진다. ⇒ **`맵으로`는 어느 시점에도 렌더된다.** 단어 선택과 갈리는
자리이고, 갈리는 이유는 그쪽에는 대신할 `결과 보기`가 있다는 것이다.
...
| 무엇 | 왜 못 정하나 | 갈 곳 |
|---|---|---|
| ~~완료 뒤 출구의 정체 (`결과 보기` vs `맵으로`)~~ **닫혔다 — LIB-244 D1** | **계약이 정했다**: 퀴즈 판정이 진행을 걸지 않으므로 결과를 넘길 곳이 없고, 출구는 **머리의 `맵으로` 하나**다. `-finish` 블록을 지웠다(§4.2.3). ⚠ **`docs/screens.md`의 미정 「진행에 거는 방식」 행은 여전히 열려 있다** — 그것과 이 행은 다른 질문이다 | 닫힘 |
...
| ~~`culture-screen.css` 머리 주석 · ADR-0022 D1 표 · `docs/e2e/culture.md` 갱신~~ **셋 다 갱신됐다 — LIB-244** | 예고한 대로 액션 행이 들어오면서 셋 다 거짓이 됐고, 같은 회차에 고쳤다 — 코드 옆 주석 둘은 구현이, ADR-0022(D2 표 주석)와 `docs/e2e/culture.md`는 문서가 졌다. ⚠ **ADR-0022의 D1·D2 「결정 문면」은 한 글자도 안 바뀌었다** — 문화 학습이 「액션 행이 없는 쪽」에서 「있는 쪽」으로 옮겨 갔을 뿐이다 | 닫힘 |
